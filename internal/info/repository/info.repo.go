...
func (r *JobRepository) GetCardData() ApiResponse {
	query := `
		 with status_data as (
            select 
                case
            ...
            case 
              ...
              when status = 'Finis' then 2
              when status = 'Delay' then 3
            else 4 end
	`
...
func (r *JobRepository) GetExportData() ApiResponse {
	query := `
		 select
                    id,
              product_type,
                    fac_site,
                    case
                    when car_no in ('EFPC_info.','SMT_info.')  then car_no 
                    else split_part(car_no, '-', 1)
                    end as car_type,
                    car_no,
                    product_name,
                    apn_no,
